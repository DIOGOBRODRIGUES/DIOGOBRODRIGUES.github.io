Crie uma apresentação de aula em HTML utilizando Reveal.js, seguindo o padrão visual e técnico da apresentação de referência anexada.
...
- Número e título da aula: [NÚMERO — TÍTULO].
- Data ou datas: [DATAS].
- Duração disponível: [DURAÇÃO].
- Perfil e conhecimentos prévios da turma: [DESCRIÇÃO].
- Conteúdos e termos que precisam ser abordados: [LISTA].
- Objetivos de aprendizagem: [OBJETIVOS].
- Relação com aulas anteriores: [RESUMO].
- Projeto ou situação prática da disciplina: [DESCRIÇÃO OU “NÃO SE APLICA”].
- Materiais de referência: [ARQUIVOS, TEXTOS OU LINKS].
- Questões que deverão aparecer integralmente: [INSERIR OU INDICAR O ARQUIVO].
- Quantidade aproximada de slides: [QUANTIDADE OU “DEFINIR CONFORME O CONTEÚDO”].
...
9. Questões de aprendizagem.
10. Correção comentada.
...
## 6. Questões e correção

Quando houver questões fornecidas:

- Preserve integralmente enunciados, assertivas e alternativas.
- Preserve imagens, tabelas e códigos necessários à resolução.
- Não altere silenciosamente o conteúdo para fazê-lo caber.
- Identifique ano, prova e número apenas quando essas informações estiverem confirmadas.
- Não apresente uma questão autoral ou adaptada como oficial.
- Se encontrar erro ou ambiguidade, sinalize separadamente.

Quando for solicitado criar questões:

- Produza questões autorais contextualizadas e adequadas ao nível da turma.
- Identifique-as como autorais.
- Utilize alternativas plausíveis e uma resposta inequivocamente correta, quando forem de escolha única.
- Avalie compreensão e aplicação, não apenas memorização.

Mostre todas as questões antes do bloco de correção. Não revele pistas visuais ou respostas no momento da aplicação.

No bloco de correção:

- Revele o gabarito apenas ao avançar.
- Explique por que a resposta está correta.
- Comente os principais erros das demais alternativas.
- Para questões discursivas, apresente elementos esperados e critérios de avaliação.

Se uma questão não couber com legibilidade, divida-a em slides de continuação claramente identificados, sem omitir conteúdo.

Quando houver preparação para o ENADE, reserve aproximadamente 15–20 minutos finais para resolução e discussão, ajustando a quantidade e a complexidade das questões ao tempo disponível.

## 7. Notas do professor
